...
//	for (i = 0; i < Count; i ++)
//	{
//		HAL_I2C_Mem_Write(&hi2c1,0x78,0x40,I2C_MEMADD_SIZE_8BIT,Data[i],1,0x100);
//	}
//
//}
...
//	OLED_WriteData(OLED_DisplayBuf[5][10]);
//	OLED_WriteData(OLED_DisplayBuf[5][11]);
//	OLED_WriteData(OLED_DisplayBuf[5][12]);
...
void OLED_page(void)
{
	...
	OLED_DisplayBuf[4][10] = 0xff;
	OLED_DisplayBuf[5][10] = 0xff;
	OLED_DisplayBuf[6][10] = 0xff;
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteData(OLED_DisplayBuf[j][X]);
		}
	}
}
...
void OLED_DelayClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, int time)
{
...
//void OLED_ShowChar_test(int16_t Page, int16_t X, char Char, uint8_t FontSize)
//{
//
